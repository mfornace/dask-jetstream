import asyncio, sys, functools, logging, os
import boto3
import fn
...
_templates = dict(i.split(maxsplit=1) for i in templates.contents.split('#script=') if i.strip())
...
def preload_watchtower(session, group, stream=None, interval=60):
    ...
    return _templates['watchtower'].format(group=repr(group), stream=repr(stream),
        interval=interval, region=session.region_name,
        access=repr(cred.access_key), secret=repr(cred.secret_key))

################################################################################

def python_shebang(python=None):
...
def scheduler_script(host, port, volume=None, python=None, preload=''):
    ...
    The script will write a config.json in the home directory (perhaps in /root).
    '''
    if volume is None:
        visit = None
        path = ''
    ...
    return python_shebang(python) + _templates['scheduler'].format(preload=preload or '', host=host, port=port, path=repr(path))
...
def worker_script(worker, scheduler, *, python=None, preload='', interfaces=('eth0', 'en0', 'ens3')):
    ...
    return python_shebang(python) + _templates['worker'].format(preload=preload or '',
