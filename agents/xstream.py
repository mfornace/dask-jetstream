...
parser = argparse.ArgumentParser(description='Launch an agent on XStream')
...
def work():
    ...
    if args.platform == 'gpu':
        settings.update({'queue': 'gpu', 'gpus': 4, 'cpus': 24, 'gpu-flags': 'enforce-binding'})
    else:
        settings.update({'queue': 'compute', 'cpus': 12, 'gpus': 0})

    ssh.remote_submit('xstream.stanford.xsede.org', settings, user='mfornace')
